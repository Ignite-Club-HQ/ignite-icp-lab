/**
 * Local equivalent of the exported `renewalReminderPromo.test.ts` suite.
...
} from '../src/lab/edgeGuards/renewalReminderClassify';
...
describe('renewal reminder promo classification', () => {
  ...
  describe('team subscriptions', () => {
    it('team sub with no stripe and no IAP transaction is promo-granted', () => {
      expect(classifyTeamSubscription({ stripe_subscription_id: null }, false)).toBe(
        'promo_granted',
      );
    });

    it('team sub with no stripe but a live IAP transaction stays paying', () => {
      expect(classifyTeamSubscription({ stripe_subscription_id: null }, true)).toBe('paying');
    });

    it('stripe-backed team sub stays paying', () => {
      expect(classifyTeamSubscription({ stripe_subscription_id: 'sub_123' }, false)).toBe(
        'paying',
      );
    });
  });
