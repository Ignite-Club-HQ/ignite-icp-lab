/**
 * Local equivalent of the exported `edgeFunctionAuthenticationContract.test.ts`.
 *
 ...
 *
 * All checks are static (source-level) over the sanitized, inert
 * `reference/backend` copies. Nothing here starts Supabase, reads a live
 * `supabase/config.toml`, or contacts a Deno runtime.
 */
import { describe, it, expect } from 'vitest';
import { readFileSync, existsSync } from 'node:fs';
import path from 'node:path';

const root = path.resolve(__dirname, '../../reference/backend/supabase');
const fnDir = (fn: string) => path.join(root, 'functions', fn);
...
const fnMdFile = (fn: string) => path.join(fnDir(fn), 'index.ts.md');
const fnExists = (fn: string) => existsSync(fnMdFile(fn));
const source = (fn: string) => readReference(fnMdFile(fn));
const configToml = readReference(path.join(root, 'config.toml.md'));
...
function firstPrivilegedIndex(src: string): number {
  ...
  const markers = ['createClient(', 'req.json(', 'new Resend(', 'serviceClient('];
  const found = markers
...
describe('edge function authentication contract (reference/backend, local)', () => {
  it.each(ALL)('%s exists and is declared verify_jwt = false', (fn) => {
    expect(fnExists(fn), `${fn} missing on disk`).toBe(true);
  ...
  it.each(ALL)('%s authenticates before any privileged work', (fn) => {
    ...
    expect(auth < privileged, `${fn} performs privileged work before authenticating`).toBe(true);
  });
  ...
  it.each(ALL)('%s returns 401 on missing/invalid credentials', (fn) => {
    const src = source(fn);
    const helper = readReference(path.join(root, 'functions/_shared/callerAuth.ts.md'));
    const internal = readReference(path.join(root, 'functions/_shared/internal-auth.ts.md'));
  ...
  it('no hardcoded JWT-shaped bearer tokens remain in the secured functions', () => {
    // The exported source's own check was redacted to `/****** ?? [];` by the
    // sanitizer that produced the authoritative bundle (an unterminated regex
    // literal — it cannot even parse). This reconstructs the documented
    // intent: no literal three-segment JWT bearer token embedded in source.
    const jwtBearerLiteral = /Bearer\s+[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/;
    for (const fn of ALL) {
      const src = source(fn);
      expect(src, `${fn} embeds a hardcoded bearer token`).not.toMatch(jwtBearerLiteral);
    }
  });
});
