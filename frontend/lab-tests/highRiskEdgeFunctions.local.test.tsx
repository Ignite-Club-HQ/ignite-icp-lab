/**
 * Local equivalent of the exported `highRiskEdgeFunctions.security.test.ts`
 * suite.
 *
 * Covers:
 *  - Association club-event fan-out: authorization before mutation,
 *    membership filtering, duplicate club-id collapsing, single atomic RPC,
 *    all-or-nothing.
 *  - Signed URL issuance: per-object authorization, mixed batches, origin
 *    validation, traversal rejection and `expiresIn` bounds.
 *
 * The fan-out control flow is reproduced faithfully (mirrors the Edge
 * Function's real validation pipeline against a synthetic RPC double); the
 * storage/signing logic is exercised directly through the local port of the
 * pure, dependency-free `_shared/storageUrlAuth.ts` +
 * `_shared/signedUrlBatch.ts` modules.
 */
...
import {
  ...
  MAX_EXPIRES_IN,
  MIN_EXPIRES_IN,
} from '../src/lab/edgeGuards/storageUrlAuth';
import { signAuthorizedBatch } from '../src/lab/edgeGuards/signedUrlBatch';
...
describe('parseStorageObjectRef', () => {
  ...
  it('rejects external URLs that merely contain a Storage marker', () => {
    expect(
      parseStorageObjectRef('https://evil.example/storage/v1/object/public/photos/a.jpg', SUPA),
    ).toBeNull();
    expect(
      parseStorageObjectRef(
        'https://proj.supabase.co.evil.example/storage/v1/object/public/photos/a.jpg',
        SUPA,
      ),
    ).toBeNull();
  });
...
function makeDeps(allow: (bucket: string, path: string) => boolean) {
  ...
  const sign = vi.fn(async (bucket: string, path: string) => ({
    url: `${SUPA}/storage/v1/object/signed/${bucket}/${path}`,
    error: null,
  }));
...
describe('signAuthorizedBatch', () => {
  ...
  it('never signs external or traversal URLs', async () => {
    const evil = 'https://evil.example/storage/v1/object/public/photos/a.jpg';
