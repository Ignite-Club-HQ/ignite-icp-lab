 ...
 * `processEventNotifications.pagination.test.ts` suite.
 *
 * Pagination + fail-closed tests for event-notification recipient
 * resolution.
 *
 * Defects covered:
 *  1. mini-league recipients truncated at the PostgREST row cap
 *  2. targeted guardian discovery truncated at the row cap
 *  3-6. team / mini-league / child-assignment / guardian read failures
 *       treated as a valid empty or partial audience
 */
...
describe('fail-closed on every recipient-source read failure', () => {
  ...
  it('the thrown error is sanitised — no DB text, SQL or URLs', async () => {
    ...
    db.errors.user_roles = {
      code: '42501',
      message: 'permission denied for table user_roles at https://db.internal/postgres',
    };
