/**
 * Local equivalent of the exported `sharedGuards.test.ts` regression suite for
 * the shared Edge Function Pro guards `requireClubPro`, `requireTeamPro`, and
 * `requireAnyClubPro`, exercised here against the local port in
 * `src/lab/edgeGuards/proAccessGuard.ts` with a synthetic RPC double instead
 * of a live Supabase client.
 */
...
} from '../src/lab/edgeGuards/proAccessGuard';
...
function makeSupabase(rpc: (name: string, args: unknown) => unknown): SupabaseRpcLike {
  return { rpc: vi.fn(rpc) } as unknown as SupabaseRpcLike;
}
...
describe('requireClubPro', () => {
  ...
  it('must convert a thrown club entitlement lookup into a safe denial response', async () => {
    const supabase = makeSupabase(() => {
      throw new Error('internal: synthetic-host/db failed: token=abc123\n at foo:42');
    });
  ...
  it('converts a rejected RPC promise (async transport failure) into 500 pro_check_failed', async () => {
    const supabase = { rpc: vi.fn(() => Promise.reject(new Error('fetch failed'))) } as unknown as SupabaseRpcLike;
...
describe('requireTeamPro', () => {
  ...
  it('converts a rejected RPC promise into 500 pro_check_failed', async () => {
    const supabase = { rpc: vi.fn(() => Promise.reject('network')) } as unknown as SupabaseRpcLike;
...
describe('requireAnyClubPro', () => {
  ...
  it('must convert a thrown any-club entitlement lookup into a safe denial response', async () => {
    const supabase = makeSupabase(() => {
      throw new Error('BACKEND_URL=https://internal-db.invalid secret=abc');
    });
    ...
    expect(body).toEqual({ error: 'pro_check_failed' });
    const raw = JSON.stringify(body);
    expect(raw).not.toContain('internal-db.invalid');
    expect(raw).not.toContain('secret');
  });

  it('converts a rejected RPC promise into 500 pro_check_failed', async () => {
    const supabase = { rpc: vi.fn(() => Promise.reject(new Error('EAI_AGAIN'))) } as unknown as SupabaseRpcLike;
