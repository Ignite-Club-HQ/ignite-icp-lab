...
## Frontend bloat/efficiency milestone update (2026-09-24)

The large-file decomposition, duplicate-code reduction, drill/training-board
removal, and safe initial-bundle deferral workstreams are now complete for the
current milestone. The remaining runtime request/subscription/render profiling
items are an optional future optimization backlog, not a blocker for this
handover audit.

Current reproducible evidence from `frontend/`:

| Metric | Current result |
| --- | ---: |
| Non-test TypeScript/TSX files under `frontend/src` | 1,394 |
| Test files under `frontend/src` | 522 |
| Non-test TypeScript/TSX source lines under `frontend/src` | 351,296 |
| Files over 1,000 lines | 59 |
| Files over 2,000 lines | 4 including generated Supabase types |
| Files over 3,000 lines | 1 (`src/integrations/supabase/types.ts`, generated) |
| Product TypeScript diagnostics | 152 |
| Product source-backed diagnostics | 92 |
| Product missing-reference diagnostics | 49 |
| Product inert Edge Function reference diagnostics | 11 |
| Quality ratchet `as any` occurrences | 1,120 |
| Quality ratchet console calls | 1,264 |
| Quality ratchet direct Supabase imports | 461 |
| Duplication ratchet scanned lines | 328,086 |
| Duplication ratchet clone groups | 1,150 |
| Duplication ratchet duplicated lines | 14,921 |
| Product JavaScript | 8,830,353 bytes |
| Largest product JavaScript chunk | 987,805 bytes |
| Product JavaScript chunks | 513 |
| Product CSS | 170,021 bytes |

The initial `product-index` JavaScript chunk was reduced from 1,112,173 bytes
to 805,870 bytes (-27.5%) by deferring lab-only ICP SDK code, synthetic fixture
data, the reset-code/zod route, app-admin demo-login UI, and the inert-by-
default legal reacceptance gate. The remaining ~5.9 KB above the original
800 KB target is accepted as core app-shell weight rather than a worthwhile
target for further churn.

Current validation evidence:

- `npm run typecheck:lab` passes.
- `npm run typecheck:product` passes against the 152-diagnostic ratchet
  baseline.
- `npm run check:isolation` passes.
- `npm run check:quality-ratchet` passes with the counts above.
- `npm run check:duplication` passes with 2,661 fewer duplicated lines than
  the ratchet baseline.
- `npm run build:product` and `npm run check:product-bundle` pass.
- The full legacy Vitest suite passed 500/500 files, 4,619 tests passed, and
  one skipped test.
- `node --test lab-tests/*.test.mjs` remains at the known baseline of 275
  passing and 4 failing tests; those failures predate the bloat/efficiency
  cleanup and are tracked separately.
...
1. **Strong regression evidence**
   - Legacy baseline: 500 files, 4,619 passing tests, 1 skipped.
   - Lab Node baseline: 275 passing tests and 4 known unrelated failing tests.
...
#### 1. Product build is available, but production-release ownership still needs CI adoption

The lab build remains the default isolated runtime (`npm run build`), and the
product/reference frontend now has a separate guarded target
(`npm run build:product`) with bundle visualization and budget enforcement
(`npm run check:product-bundle`). This is sufficient for reproducible product
bundle evidence, but it is not yet a production-release sign-off because it
still needs to be wired into CI/release gates with the broader validation
matrix.

**Required before production ownership transfer:**

- keep the current lab build and allowlist unchanged;
- keep product environment validation fail-closed;
- archive product bundle visualization/budget output in CI;
- run the product build, product typecheck, lab typecheck, isolation, quality
  ratchets, and regression suites in CI before release;
- document which product routes remain reference/porting source versus
  provider-neutral hybrid runtime.

#### 2. Provider neutrality is incomplete

There are 461 direct Supabase imports in the current quality ratchet. This is consistent with the documented coexistence state, but it means a vendor cannot safely switch a country, club, or domain to ICP by changing configuration alone.
...
`strict`, `strictNullChecks`, `noImplicitAny`, `noUnusedLocals`, and `noUnusedParameters` are disabled. The current quality ratchet reports 1,120 `as any` assertions.
...
#### 4. Lint gate is narrow

ESLint configuration and a `lint` script now exist, but the script is scoped to
selected observability/boundary files rather than the full frontend. Import
placement, floating promises, and unsafe TypeScript patterns therefore remain
only partially enforced across the full codebase.
...
- the narrow lint script and any future expanded lint targets;
- the full product build;
...
5. The product TypeScript backlog is inventoried and ratcheted: the current baseline is 152 diagnostics (92 source-backed, 49 missing-reference, and 11 inert Edge Function references), and new diagnostics fail the product check.
6. Lab-only ICP SDK code, synthetic fixture data, the reset-code/zod route, app-admin demo-login UI, and the inert-by-default legal reacceptance gate are no longer part of the initial product chunk.
7. Existing behavior was preserved with local `Suspense` boundaries, unchanged data-access contracts, and the legacy suite passing with 4,619 tests passed and one skipped.
...
#### P0 - keep measurable budgets in release gates

1. Run and archive the existing product build target (`npm run build:product`).
2. Record:
   - initial JS and CSS;
...
3. Enforce budgets in CI. Do not optimize from source line count alone.
...
#### P1 - split future large pages only when a concrete risk remains

The current large-file milestone is accepted as complete. For any future
oversized page that still blocks safe feature work, extract in this order:
...
#### P1 - converge chat orchestration when runtime profiling justifies it
...
The strongest assets are the regression suites, hybrid planning documents, lab isolation, the guarded product build/bundle budget, quality and duplication ratchets, and existing performance-oriented chat infrastructure. The principal risks are incomplete provider-neutral migration, permissive TypeScript configuration, narrow lint coverage, remaining direct Supabase surfaces, and runtime profiling that is still optional backlog rather than CI-enforced release evidence.
