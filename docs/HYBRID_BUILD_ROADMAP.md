...
- ICP mode must use a typed authenticated actor/service, an explicit
  unavailable/read-only state, or clearly labelled synthetic data for local
  demonstrations; synthetic data must never be presented as persisted
  production state;
...
- Media now uses the authenticated local `media_metadata` actor for asset,
  reaction, and comment reads/writes when configured; if it is unavailable,
  the page may show clearly labelled synthetic data for local demonstrations,
  without claiming persistence. Protected object storage, upload, moderation,
  retention, and child-media privacy remain outside this frontend slice and
  must be connected before those capabilities are enabled.
