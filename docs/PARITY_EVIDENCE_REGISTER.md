...
| --- | --- | --- | --- |
| `identity_access` | `frontend/scripts/test-identity-access.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/identity_access/src/lib.rs` | account registration, linking, revocation, scoped site roles, exact-team direct-role membership, exclusions, consent, anonymous rejection, account-owned record erasure cleanup, live disposable erasure probe | profile privacy, recovery, complete source helper matrix |
...
| `events_domain` | `frontend/scripts/test-domain-canisters.mjs`, `frontend/scripts/test-events-motoko-comparison.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/events_domain/src/main.mo` | event ownership, RSVP, attendance, duties, roster, recurrence, revision fencing, stale-write rejection, Motoko representative workflow | child visibility parity, timer callback capability, full event RLS, scale/pagination |
| `competition_domain` | `frontend/scripts/test-domain-canisters.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/competition_domain/src/main.mo` | team entries, seasons, archive fencing, fixtures, results, scoped join-token authorization, expiry validation, replay protection | league ownership, officials/coordinators/participants, archive mutation parity, full competition RLS |
| `messaging_domain` | `frontend/scripts/test-domain-canisters.mjs`, `frontend/scripts/test-product-motoko-canisters.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/messaging_domain/src/main.mo` | participant reads, ordering, idempotency, bounded pages, stale cursors, unread, receipts, author-only team updates, author/moderator deletion, directional block/unblock enforcement | groups/DMs, reactions/replies/polls/moderation/retention, scale |
| `media_metadata` | `frontend/scripts/test-domain-canisters.mjs`, `frontend/scripts/test-product-motoko-canisters.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/media_metadata/src/main.mo` | child-sensitive classification, encrypted metadata flag, owner/governor-only metadata reads, delegated capability holders, purpose/expiry validation, expired-capability rejection, outsider rejection, deletion/retention | album/uploader/commenter/guardian parity, bytes/chunks/scanning/moderation |
