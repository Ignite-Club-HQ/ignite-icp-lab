...
| `messaging_domain` | `frontend/scripts/test-domain-canisters.mjs`, `frontend/lab-tests/rls-parity-matrix.test.tsx`, `backend/messaging_domain/src/lib.rs` | participant reads, ordering, idempotency, bounded pages, stale cursors, unread, receipts, author/moderator deletion, directional block/unblock enforcement | groups/DMs, reactions/replies/polls/moderation/retention, scale, live Rust domain probe |
